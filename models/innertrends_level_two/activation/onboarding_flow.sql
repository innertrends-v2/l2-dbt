...
ORDERED_STEPS AS (
    SELECT
        ...
        ROW_NUMBER() OVER (PARTITION BY data.ACCOUNT_ID ORDER BY data.TIMESTAMP, step_order.STEP_INDEX) AS RN
...
CLEANED_STEPS AS (
  ...
  WHERE NOT EXISTS (
    ...
    WHERE O2.ACCOUNT_ID = C.ACCOUNT_ID
      AND (O2.TIMESTAMP > C.TIMESTAMP OR (O2.TIMESTAMP = C.TIMESTAMP AND O2.STEP_INDEX > C.STEP_INDEX))
      AND (O2.TIMESTAMP < O.TIMESTAMP OR (O2.TIMESTAMP = O.TIMESTAMP AND O2.STEP_INDEX < O.STEP_INDEX))
...
WITH_LAG AS (
    SELECT
        *,
        LAG(ONBOARDING_STEP) OVER (PARTITION BY ACCOUNT_ID ORDER BY TIMESTAMP, STEP_INDEX) AS FROM_STEP,
        LAG(TIMESTAMP) OVER (PARTITION BY ACCOUNT_ID ORDER BY TIMESTAMP, STEP_INDEX) AS FROM_TIMESTAMP,
        LAG(STEP_INDEX) OVER (PARTITION BY ACCOUNT_ID ORDER BY TIMESTAMP, STEP_INDEX) AS FROM_STEP_INDEX
    FROM CLEANED_STEPS
),
